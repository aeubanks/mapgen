...
namespace sdl2 {
//	SDLTextureWrapper::SDLTextureWrapper(SDL_Renderer * renderer, SDL_Surface * surface): texture_(SDL_CreateTextureFromSurface(renderer, surface)), width_(surface->w), height_(surface->h) {
//		if (!texture_) {
//			throw sdl2_error("SDL_CreateTextureFromSurface");
//		}
//	}

	SDLTextureWrapper::SDLTextureWrapper(SDLRendererWrapper & renderer, SDLSurfaceWrapper & surface) : texture_(SDL_CreateTextureFromSurface(renderer.get(), surface.get()), SDL_DestroyTexture) /* SDLTextureWrapper(renderer.get(), surface.get())*/ {
		if (!texture_) {
			throw sdl2_error("SDL_CreateTextureFromSurface");
		}
	...
	SDLTextureWrapper::SDLTextureWrapper(SDLRendererWrapper & renderer, SDLSurfaceWrapper && surface): SDLTextureWrapper(renderer, surface) {	}
	...
	void SDLTextureWrapper::unset_color_mod() {
		set_color_mod(0xFF, 0xFF, 0xFF);
	}

	void SDLTextureWrapper::unset_alpha_mod() {
		set_alpha_mod(0xFF);
	}
