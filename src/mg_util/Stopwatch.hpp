...
namespace mg_util {
	...
	public:
		...
		using time_point_t = decltype(now()); // std::chrono::time_point<std::chrono::steady_clock>;
		using time_dif_t = decltype(std::chrono::operator-(time_point_t(), time_point_t()));
		...
		// print now() - begin_time_ with a description, then update prev_time_ to now()
