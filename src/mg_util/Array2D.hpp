...
namespace mg_util {
	...
	class Array2D {
		...
		public:
			Array2DCoordIterator_(Array2DCoordIterator_ & other) : array_(other.array_), cur_coord_(other.cur_coord_) { }

			Array2DCoordIterator_(Array2DCoordIterator_ && other) : array_(other.array_), cur_coord_(other.cur_coord_) { }

			Array2DCoordIterator_(const Array2D<T> & arr, Coord2D coord) : array_(arr), cur_coord_(coord) { }
		...
		public:
			...
			Array2DCoordIterator_ begin() { return Array2DCoordIterator_(array, Coord2D(0, 0)); }

			Array2DCoordIterator_ end() { return Array2DCoordIterator_(array, Coord2D(0, array.height())); }
		...
		// ValueType is meant to be either 'T' or 'const T' depending on the application
		...
		public:
			...
			Array2DCoordValuePairTemplate_(Coord2D c, ValueType & val) : coord(c), value(val) { }
		};
		...
		// an iterator for an Array2D that provides all of the coords and values through Array2DCoordValuePairTemplate_
		...
		public:
			...
			Array2DCoordValueIteratorTemplate_(ArrayType & arr, Coord2D coord) : array_(arr), cur_coord_(coord) { }
		...
		using Array2DCoordValueIterator_ = Array2DCoordValueIteratorTemplate_<Array2D<T>, Array2DCoordValuePair_>;
		using Array2DCoordValueIteratorConst_ = Array2DCoordValueIteratorTemplate_<const Array2D<T>, Array2DCoordValuePairConst_>;
	...
	public:
		Array2D(int width, int height, T init_val = T()) : width_(width), height_(height), values_(static_cast<unsigned long>(width * height), init_val) { }
		...
		typename decltype(values_)::reference operator[](Coord2D coord) {
			if (!in_bounds(coord)) {
				throw mg_error(fmt::format("out of bounds: accessing {} (bounds {}, {})", coord, width_, height_));
			}
		...
		Array2DCoordValueIterable_ coords_values() { return Array2DCoordValueIterable_(*this); }
		Array2DCoordValueIterableConst_ coords_values() const { return Array2DCoordValueIterableConst_(*this); }
	};
