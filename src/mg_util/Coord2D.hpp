...
namespace mg_util {
	...
	public:
		...
		// basically, can this coord be used to index an Array2D with the specified width/height
...
namespace std {
	...
	struct hash<mg_util::Coord2D> {
		...
		std::size_t operator()(const mg_util::Coord2D & coord) const {
			std::size_t seed = 0;
			seed ^= static_cast<decltype(seed)>(coord.x) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
			seed ^= static_cast<decltype(seed)>(coord.y) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
			return seed;
		}
	};
}
