...
namespace lodepng {	
	std::vector<unsigned char> map_to_image(mapgen::Map & map, int tile_size_i) {
		...
		std::vector<unsigned char> image(width * height * tile_size * tile_size * 4, 0xFF);
		for (auto && pair : map.coords_values()) {
			unsigned char val = (pair.value.type == mapgen::MapTileType::Ground) ? static_cast<unsigned char>(0xFF) : static_cast<unsigned char>(0x00);
	...
	void save_map_as_image(std::string file_name, mapgen::Map & map, int tile_size) {
		...
		auto err = lodepng::encode(file_name, image, static_cast<unsigned>(map.width() * tile_size), static_cast<unsigned>(map.height() * tile_size));
		fmt::println_obj(err);
