...
namespace mapgen {
	...
	protected:
		// a reference to a Random so that if multiple MapGenerators are used, they can use the same one and be deterministic
		mg_util::Random & rand_;

		// destructor is protected so that subclasses cannot be destroyed through a pointer to the base class
		~MapGenerator() {}
	};
	...
	void _gen_helper(Map & map, T && t, Ts &&... args) {
		static_assert(std::is_base_of<MapGenerator, typename std::remove_reference<T>::type>::value, "must use subclasses of MapGenerator in create_map()");
		t(map);
