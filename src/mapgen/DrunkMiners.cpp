...
void DrunkMiners::modify_map(Map & map) {
    bool is_large_wall;
    do {
        std::vector<Coord2D> miners{Coord2D(rand_.rand_int_exc(0, map.width()), rand_.rand_int_exc(0, map.height()))};
        while (!miners.empty()) {
            for (int32_t i = miners.size() - 1; i >= 0; --i) {
                auto miner = miners[i];
                // duplicate miner
                if (rand_.rand_bool(spawn_prob_)) {
                    miners.push_back(miner);
                }
                int32_t num_ground_neighbors = 8 - map.num_neighbors_of_type_moore(MapTileType::Wall, miner);
        ...
        is_large_wall = false;
        auto lambda = [&is_large_wall](Map & map2, std::vector<Coord2D> & section) {
            auto map_size =
