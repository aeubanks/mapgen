...
namespace mapgen {
	void DrunkMiners::modify_map(Map & map) {
		bool is_large_wall;
		do {
			vector<Coord2D> miners{{rand_.rand_int_exc(0, map.width()), rand_.rand_int_exc(0, map.height())}};
			while (!miners.empty()) {
			...
			auto lambda = [&is_large_wall](Map & map2, vector<Coord2D> & section) {
				auto map_size = static_cast<decltype(section.size())>(map2.width() * map2.height());
