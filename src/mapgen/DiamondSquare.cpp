...
namespace mapgen {
	DiamondSquare::DiamondSquare(mg_util::Random & r, double smoothness) : MapGenerator(r), smoothness_(smoothness) { }
	...
	void DiamondSquare::modify_map(Map & map) {
		...
		int inc = std::min(mg_util::pow2rounddown(width), mg_util::pow2rounddown(height));
	...
	void DiamondSquare::init_vals(int inc, double deviation, Array2D<double> & values) {
	...
	void DiamondSquare::divide(int inc, double deviation, Array2D<double> & values, Map & map) {
	...
	void DiamondSquare::square(int x, int y, int inc, double deviation, Array2D<double> & values, Map & map) {
		double rand_val = rand_.rand_double(-deviation, deviation);
		double average = average_of_valid_coords({x + inc, y + inc}, {x - inc, y - inc}, {x - inc, y + inc}, {x + inc, y - inc}, values, map);
		values[{x, y}] = rand_val + average;
	}

	void DiamondSquare::diamond(int x, int y, int inc, double deviation, Array2D<double> & values, Map & map) {
		double rand_val = rand_.rand_double(-deviation, deviation);
		double average = average_of_valid_coords({x + inc, y}, {x - inc, y}, {x, y + inc}, {x, y - inc}, values, map);
		values[{x, y}] = rand_val + average;
	}

	double DiamondSquare::average_of_valid_coords(Coord2D a, Coord2D b, Coord2D c, Coord2D d, Array2D<double> & values, Map & map) {
	...
	void DiamondSquare::scale_values(Array2D<double> & values) {
		...
		if (dif == 0.0) {  // if min == max then just set everything to 0
			...
			values.for_each([min, dif](double & val) {
				val = 0.0;
			});
		} else { // else scale all values to fit between 0.0 - 1.0
			double scale = 1.0 / dif;
			values.for_each([min, scale](double & val) {
				val = (val - min) * scale;
			});
		}
	}

	void DiamondSquare::values_to_map(double threshold, Array2D<double> & values, Map & map) {
