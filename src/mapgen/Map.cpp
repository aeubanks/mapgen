...
namespace mapgen {
	...
	int Map::num_neighbors_von_neumann(int dist) {
	...
	Map::Map(int width, int height, MapTileType type) : Map(width, height, false, type) { }

	Map::Map(int width, int height, bool wrap) : Map(width, height, wrap, MapTileType::Wall) { }

	Map::Map(int width, int height, bool wrap, MapTileType type) : map_(width, height, type), wrap_(wrap) {
		assert(width > 0);
		assert(height > 0);
	}
	...
	int Map::num_neighbors_of_type_von_neumann(MapTileType type, Coord2D coord, int dist) const {
		...
		int count = 0;
		neighbors_von_neumann_for_each(coord, dist, [&count, type, this](Coord2D neighbor) {
			if (operator[](neighbor).type == type) {
				++count;
			}
		});
	...
	int Map::num_neighbors_of_type_von_neumann(MapTileType type, Coord2D coord) const {
	...
	vector<Coord2D> Map::find_section_helper(Coord2D coord, vector<Coord2D>(Map::* neighbors_func)(Coord2D) const) const {
		assert(in_bounds(coord));
	...
	void Map::flood_fill_helper(Coord2D coord, MapTileType flood_type, vector<Coord2D>(Map::* neighbors_func)(Coord2D) const) {
		...
		if (overwrite_type == flood_type) {  // if flooding with the same type that's already there, exit
			return;
		}
...
//		return ret;
	...
	vector<Coord2D> Map::neighbors_helper(Coord2D coord, int dist, NeighborsForEachFuncType find_neighbors_func) const {
		vector<Coord2D> ret;
		auto add_to_ret = [&ret, this](Coord2D add_coord) {
			ret.push_back(add_coord);
		};
	...
	void Map::neighbors_moore_for_each(Coord2D coord, NeighborsFuncType neighbors_func) const {
	...
	void Map::neighbors_moore_weighted_for_each(Coord2D coord, int dist, NeighborsWeightedFuncType neighbors_func) const {
	...
	void Map::neighbors_von_neumann_for_each(Coord2D coord, NeighborsFuncType neighbors_func) const {
	...
	void Map::neighbors_von_neumann_weighted_for_each(Coord2D coord, int dist, NeighborsWeightedFuncType neighbors_func) const {
		auto find_weight_and_neighbors_func = [coord, dist, neighbors_func, this](Coord2D idx) {
			auto dif = coord - idx;
	...
	void Map::sections_for_each_von_neumann(MapTileType type, std::function<void(Map &, vector<Coord2D> &)> f) {
		sections_for_each_helper(type, f, &Map::find_section_von_neumann);
	}

	void Map::sections_for_each_moore(MapTileType type, std::function<void(Map &, vector<Coord2D> &)> f) {
	...
	void Map::sections_for_each_helper(MapTileType type, std::function<void(Map &, vector<Coord2D> &)> f, SectionFuncType section_func) {
		Array2D<bool> visited(width(), height());
		for (auto && item : this->coords_values()) {
			if (!visited[item.coord] && item.value.type == type) {
				auto section = (this->*section_func)(item.coord);
				std::for_each(section.begin(), section.end(), [&visited](const Coord2D & coord) { visited[coord] = true; });
				f(*this, section);
			}
		}
	}
