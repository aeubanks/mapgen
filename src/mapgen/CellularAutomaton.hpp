...
namespace mapgen {
	...
	public:
		...
		CellularAutomaton(mg_util::Random & r, int generations, CARule rule = CARule()) : MapGenerator(r), generations_(generations), rule_(rule) { }

		void modify_map(Map & map) override;

	private:
		// runs one generation on the map
		// goes through every coord, calls rule_(map, coord), if true, sets that to Ground; if false, sets that to Wall
...
//		fmt::print("{}{}\n", map, string(static_cast<unsigned int>(width), '-'));
	...
	// if a tile is alive, returns true if the number of alive neighbors >= alive_limit_
	// if a tile is dead, returns true if the number of alive neighbors >= dead_limit_
	class CARuleAliveDeadLimit {
	public:
		CARuleAliveDeadLimit(int alive_limit, int dead_limit): alive_limit_(alive_limit), dead_limit_(dead_limit) {}
