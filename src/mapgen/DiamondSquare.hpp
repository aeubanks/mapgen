...
namespace mapgen {
	...
	private:
		...
		// averages the square with length 2 * inc with x, y as the center's corner values, adds a random deviation, and sets map[x, y]
		void square(int x, int y, int inc, double deviation, Array2D<double> & values, Map & map);
		// averages the diamond with length 2 * inc with x, y as the center's corner values, adds a random deviation, and sets map[x, y]
		void diamond(int x, int y, int inc, double deviation, Array2D<double> & values, Map & map);
		// returns the average of the 4 coords, ignoring any out of bounds coords
		double average_of_valid_coords(Coord2D a, Coord2D b, Coord2D c, Coord2D d, Array2D<double> & values, Map & map);
		// scales all values to between 0.0-1.0
		void scale_values(Array2D<double> & values);
		// for each coord, if the corresponding value is above the threshold, the tile is ground, otherwise wall
